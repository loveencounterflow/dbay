(function() {
  'use strict';
  var DBay, DBay_ctx, DBay_openclose, DBay_query, DBay_sqlgen, DBay_sqlx, DBay_stdlib, DBay_udf, E, FS, GUY, GUY_LEGACY, H, LFT, PATH, Random, SQL, alert, debug, echo, guy, help, info, inspect, log, new_bsqlt3_connection, plain, praise, rpr, sql, types, urge, warn, whisper;
  ...
  DBay = (function() {
    ...
    class DBay extends DBay_query(DBay_ctx(DBay_openclose(DBay_stdlib(DBay_sqlgen(DBay_udf(Function)))))) {
      ...
      static cast_constructor_cfg(me, cfg = null) {
        var R/* TAINT should not be needed */, clasz, filename;
        ...
        R = H.omit_nullish(R);
        R = {...this.C.defaults.constructor_cfg, ...R};
        return R;
      }
      ...
      constructor(cfg) {
        ...
        this._me.state = LFT.freeze({});
        H.hide(this._me, 'SQL', SQL);
        H.hide(this._me, 'sql', sql);
        H.hide(this._me, 'rnd', new Random({
        ...
        //.......................................................................................................
        H.hide(this._me, '_dbs', {});
        H.hide(this._me, 'E', E);
        this._me._register_schema('main', this._me.cfg.path, this._me.cfg.temporary);
        if (!this.constructor._skip_sqlt) {
          H.hide(this._me, 'sqlt1', this._me._new_bsqlt3_connection());
          this.set_journal_mode.call(this._me, this._me.cfg.journal_mode);
        }
        H.hide(this._me, 'macros', new DBay_sqlx());
        if (typeof this._compile_sql === "function") {
          this._compile_sql();
        }
        /* make `alt` an on-demand clone of present instance: */
        H.def_oneoff(this._me, 'alt', {
          enumerable: false
        }, () => {
