(function() {
  'use strict';
  var CND, E, H, SQL, add_views, badge, debug, echo, freeze, guy, help, info, lets, rpr, urge, warn, whisper;
  ...
  this.DBay_trash = (clasz = Object) => {
    return class extends clasz {
      ...
      //---------------------------------------------------------------------------------------------------------
      trash(cfg) {
        if (this._trash_select_from_statements == null) {
          guy.props.hide(this, '_trash_select_from_statements', SQL`select * from dbay_create_table_statements;`);
        }
        this.types.validate.dbay_trash_cfg((cfg = {...this.constructor.C.defaults.dbay_trash_cfg, ...cfg}));
        this._implement_trash();
        switch (cfg.format) {
          case 'rows':
            if ((cfg.path != null) && cfg.path !== false) {
              throw new E.DBay_argument_not_allowed('^dbay/trash@1^', 'path', cfg.path);
            }
            return this.query(this._trash_select_from_statements);
          case 'sql':
            if (cfg.path == null) {
              throw new E.DBay_argument_missing('^dbay/trash@2^', 'path');
            }
            if (cfg.path === false) {
              return (this.query(this._trash_select_from_statements)).join('\n');
            }
            return this._trash_to_file(cfg.path);
          case 'sqlite':
      ...
      _trash_to_file(path) {
        ...
        ref = this(SQL`select * from dbay_create_table_statements;`);
        for (row of ref) {
