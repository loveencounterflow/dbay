(function() {
  'use strict';
  var CND, E, FS, H, PATH, SQL, add_views, badge, debug, echo, freeze, guy, help, info, lets, rpr, urge, warn, whisper;
  ...
  this.DBay_trash = (clasz = Object) => {
    return class extends clasz {
      ...
      trash_to_sql(cfg) {
        ...
        return this._trash_with_fs_open_do(path, overwrite, ({path, fd}) => {
          for (row of iterator) {
      ...
      trash_to_sqlite(cfg) {
        ...
        var buffer, fd, overwrite, path, sql, sqlt;
        ...
        sqlt.exec(sql);
        if ((cfg.path == null) || (cfg.path === false)) {
          return (buffer = sqlt.serialize());
        }
        ({path, fd} = this._trash_open(path, overwrite));
        return this._trash_with_fs_open_do(path, overwrite, ({path, fd}) => {
      ...
      _trash_with_fs_open_do(path, overwrite, fn) {
        var R, fd;
        path = this._trash_get_path(path);
