(function() {
  ...
  this.declare('constructor_cfg', {
    tests: {
      ...
      "@isa.dbay_journal_mode x.journal_mode": function(x) {
        return this.isa.dbay_journal_mode(x.journal_mode);
      }
    }
  });
  ...
  //-----------------------------------------------------------------------------------------------------------
  this.declare('dbay_journal_mode', {
    tests: {
      "x in [ 'delete', 'wal', 'truncate', 'persist', 'memory', 'wal', 'off', ]": function(x) {
        return x === 'delete' || x === 'wal' || x === 'truncate' || x === 'persist' || x === 'memory' || x === 'wal' || x === 'off';
      }
    }
  });
