(function() {
  ...
  // TRASH
  //-----------------------------------------------------------------------------------------------------------
  this.declare('dbay_trash_cfg', function(x) {
    return {
      "@isa.object x": function(x) {
        return this.isa.object(x);
      },
      "x.format in [ 'rows', 'sql', 'sqlite', ]": function(x) {
        var ref;
        return (ref = x.format) === 'rows' || ref === 'sql' || ref === 'sqlite';
      },
      "@isa_optional.nonempty_text x.path or a boolean": function(x) {
        if (x.path == null) {
          return true;
        }
        if (this.isa.boolean(x.path)) {
          return true;
        }
        if (this.isa.nonempty_text(x.path)) {
          return true;
        }
        return false;
      }
