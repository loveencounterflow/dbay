(function() {
  'use strict';
  /* https://day.js.org */
  var CND, E, GUY, badge, dayjs, debug, echo, help, info, rpr, urge, walk_split_parts, warn, whisper;
  ...
  this.DBay_stdlib = (clasz = Object) => {
    return (function() {
      ...
      _Class = class extends clasz {
        ...
        constructor() {
          super();
          GUY.props.hide(this, '_dayjs', dayjs);
          return void 0;
        }
        ...
        create_stdlib() {
          ...
          this.create_function({
            ...
            varargs: false,
            call: (dbay_timestamp) => {
              return this.dt_from_now(dbay_timestamp);
            }
          });
          //-------------------------------------------------------------------------------------------------------
          return null;
        ...
        dt_from_now(dbay_timestamp) {
          return (this.dt_parse(dbay_timestamp)).fromNow();
        }

        //---------------------------------------------------------------------------------------------------------
        dt_now() {
          return this._dayjs().utc().format(this._dt_dbay_timestamp_output_template);
        }

        //---------------------------------------------------------------------------------------------------------
        dt_parse(dbay_timestamp) {
          var R;
          this.types.validate.dbay_dt_timestamp(dbay_timestamp);
          R = (this._dayjs(dbay_timestamp, this._dt_dbay_timestamp_input_template)).utc();
          if (!this.types.isa.dbay_dt_valid_dayjs(R)) {
            throw new E.DBay_invalid_timestamp('^dbay/stdlib@1^', dbay_timestamp);
          }
          return R;
        }
      ...
      // DATETIME (2)
      //---------------------------------------------------------------------------------------------------------
      _Class.prototype._dt_dbay_timestamp_input_template = 'YYYYMMDD-HHmmssZ';

      _Class.prototype._dt_dbay_timestamp_output_template = 'YYYYMMDD-HHmmss[Z]';
